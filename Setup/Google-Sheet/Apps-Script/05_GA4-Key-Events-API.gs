...
function getGA4KeyEvents() {
  try {
    if (!ga4PropertyIDEvents) {
      SpreadsheetApp.getUi().alert("GA4 Property ID is not set in Events Sheet.");
      return;
    }
    ...
    do {
      const response = keyEventAPI.list(
        `properties/${ga4PropertyIDEvents}`,
    ...
    SpreadsheetApp.getActiveSpreadsheet().toast(
      'GA4 Key Events was downloaded successfully',
      'Done',
      5
    );
  } catch (err) {
...
function createGA4KeyEvent() {
  try {
    ...
    eventRows.forEach((row, index) => {
      const event = {
        eventName: row[eventNameColumn - 1] ? row[eventNameColumn - 1].trim() : '',
        countingMethod: row[keyEventCountingColumn - 1] ? row[keyEventCountingColumn - 1].trim() : 'ONCE_PER_EVENT',
        checkbox: row[eventEditCheckboxColumn - 1],
        length: index // used later to map back to sheet row
      };
      sheetData.push(event);
    });

    // Only rows where the checkbox is checked
    const selectedEvents = sheetData.filter(row => row.checkbox);

    // Split into valid and invalid based on event name length
    const invalidEvents = [];
    const validEvents = [];

    selectedEvents.forEach(row => {
      const nameLength = row.eventName ? row.eventName.length : 0;
      if (nameLength < 1 || nameLength > 40) {
        invalidEvents.push(row);
      } else {
        validEvents.push(row);
      }
    });

    // If there are invalid events, inform the user
    if (invalidEvents.length > 0) {
      const messageLines = invalidEvents.map(row => {
        const rowIndex = row.length + headerRowNumber + 1; // convert index to sheet row
        const eventName = row.eventName || '(empty)';
        const nameLength = row.eventName ? row.eventName.length : 0;
        return 'Row ' + rowIndex + ': "' + eventName + '" (length: ' + nameLength + ')';
      });

      ui.alert(
        'Some selected events were NOT created as Key Events.\n\n' +
        'Event names must be between 1 and 40 characters.\n\n' +
        messageLines.join('\n')
      );
    }

    // If there are no valid events, stop here
    if (validEvents.length === 0) {
      return;
    }

    // Create Key Events only for valid events
    ...
    // Fetch Key Events and update sheet
    let pageToken;
    let keyEvents = [];
    const range = ss.getSheetByName(helperDropDownTab).getRange('HelperKeyEventType');
    const rule = SpreadsheetApp.newDataValidation().requireValueInRange(range).build();
    ...
    ui.alert("Key Event(s) were created.");
  } catch (err) {
