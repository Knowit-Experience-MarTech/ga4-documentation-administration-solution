...
function listGA4Accounts() {
  try {
      ...
      if (response.accountSummaries && response.accountSummaries.length) {
        for (let i = 0; i < response.accountSummaries.length; i++) {
          ...
          if (account.propertySummaries) {
            const properties = AnalyticsAdmin.Properties.list({ filter: 'parent:' + account.account });
    ...
    if (accountList && accountList.length) {
      ...
      const startRow = 2;
      const numRows = sheet.getLastRow(); // The number of rows to clear
      ...
      const message = accountCount + ' Google Analytics Accounts are available in the account dropdown.';
  ...
  try {
    ...
    if (ga4AccountID) {
      ...
      if (propertyList && propertyList.length) {
        ...
        const message = propertyCount + ' Google Analytics Properties are available in the property dropdown.';
        ss.toast(message, 'Finished', 5); // 5 seconds
      }
    }
  } catch (err) {
