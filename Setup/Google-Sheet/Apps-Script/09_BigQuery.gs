...
function uploadEventsToBigQueryManually() {
  try {
    ...
    var success = uploadEventsToBigQuery(); // returns true/false now
    if (success) {
      ss.toast(
        'Event and parameter documentation successfully uploaded to BigQuery.',
        'Upload complete',
        5
      );
    }
    ...
    if (manualBQTrigger) {
      SpreadsheetApp.getUi().alert('Error occurred in "uploadEventsToBigQueryManually" function: \n' + err);
    }
  } finally {
    manualBQTrigger = false;
  }
}
...
function uploadAnnotationsToBigQueryManually() {
  try {
    manualBQTrigger = true;
    var success = uploadAnnotationsToBigQuery(); // returns true/false now
    if (success) {
      ss.toast(
        'Annotations successfully uploaded to BigQuery.',
        'Upload complete',
        5
      );
    }
  } catch (err) {
    Logger.log('uploadAnnotationsToBigQueryManually: ' + err.stack);
    if (manualBQTrigger) {
      SpreadsheetApp.getUi().alert('Error occurred in "uploadAnnotationsToBigQueryManually" function: \n' + err);
    }
  } finally {
    manualBQTrigger = false;
  }
...
function getTableSchema(sheetName) {
  const bqSettingsSchema = {
    fields: [
      {name: 'events_exclusion', type: 'STRING', description: "String containing events to exclude from analysis."},
      {name: 'parameters_exclusion', type: 'STRING', description: "String containing parameters to exclude from anomaly detection."},
      {name: 'ep_day_interval_short', type: 'INT64', description: "Integer representing the short day interval for event parameter analysis."},
      {name: 'ep_day_interval_extended', type: 'INT64', description: "Integer representing the extended day interval for event parameter analysis."},
      {name: 'ep_delete_event_count_after_days', type: 'INT64', description: "Integer representing the number of days after which event counts are deleted."},
      {name: 'anomaly_day_interval_short', type: 'INT64', description: "Integer representing the short day interval for anomaly detection."},
      {name: 'anomaly_day_interval_extended', type: 'INT64', description: "Integer representing the extended day interval for anomaly detection."},
      {name: 'anomaly_days_before_anomaly_detection', type: 'INT64', description: "Integer representing the number of days before anomaly detection starts."},
      {name: 'anomaly_day_interval_large', type: 'INT64', description: "Integer representing the large day interval for anomaly detection."},
      {name: 'anomaly_delete_anomaly_data_after_days', type: 'INT64', description: "Integer representing the number of days after which anomaly data is deleted."},
      {name: 'anomaly_day_interval_new_events_params', type: 'INT64', description: "Integer representing the day interval for new events parameters in anomaly detection."},
      {name: 'anomaly_stddev_model_setting', type: 'STRING', description: "String indicating the standard deviation model setting for anomaly detection."},
      {name: 'anomaly_min_expected_count', type: 'INT64', description: "Integer representing the minimum expected count for anomaly detection."},
      {name: 'anomaly_stddev_multiplier', type: 'FLOAT64', description: "Float representing the standard deviation multiplier for anomaly detection."},
      {name: 'anomaly_events_explained_by_sessions_threshold', type: 'FLOAT64', description: "Float representing the threshold for events explained by sessions in anomaly detection."},
      {name: 'anomaly_parameters_explained_by_sessions_threshold', type: 'FLOAT64', description: "Float representing the threshold for parameters explained by sessions in anomaly detection."},
      {name: 'events_anomaly_exclusion', type: 'STRING', description: "String containing events to exclude from anomaly detection."}
    ]
  };

  const eventSchema = {
    fields: [
      {name: 'event_group', type: 'STRING', description: "String representing the group or category to which the event belongs."},
      {name: 'event_name', type: 'STRING', description: "String representing the name of the event."},
      {name: 'event_method', type: 'STRING', description: "String describing the method or technique used to trigger the event."},
      {name: 'event_type', type: 'STRING', description: "String indicating the type or category of the event."},
      {name: 'key_event', type: 'BOOL', description: "Boolean value indicating if the event is considered a key event."},
      {name: 'key_event_counting', type: 'STRING', description: "String describing how the event is counted as a key event."},
      {name: 'event_description', type: 'STRING', description: "String providing a detailed explanation of the event."},
      {name: 'event_parameters', type: 'STRING', description: "String containing parameters associated with the event."},
      {name: 'event_item_parameters', type: 'STRING', description: "String containing item-specific parameters associated with the event."},
      {name: 'event_user_parameters', type: 'STRING', description: "String containing user-defined parameters associated with the event."},
      {name: 'event_comment', type: 'STRING', description: "String containing general comments or notes about the event."},
      {name: 'event_image_documentation', type: 'STRING', description: "String containing a link or reference to image documentation for the event."},
      {name: 'event_edited_time', type: 'DATETIME', description: "Timestamp indicating when the event documentation was last modified."},
      {name: 'event_website', type: 'BOOL', description: "Boolean value indicating if the event is relevant to a website."},
      {name: 'event_ios_app', type: 'BOOL', description: "Boolean value indicating if the event is relevant to an iOS application."},
      {name: 'event_android_app', type: 'BOOL', description: "Boolean value indicating if the event is relevant to an Android application."},
      {name: 'event_ga4_api_resource_name', type: 'STRING', description: "String representing the GA4 API resource name associated with the event."},
      {name: 'event_gtm_comment', type: 'STRING', description: "String containing comments related to the event within Google Tag Manager."},
      {name: 'event_uploaded_to_bq_time',type: 'DATETIME', description: "Timestamp indicating when the event documentation was uploaded to BigQuery."}
    ]
  };

  const parameterSchema = {
    fields: [
      {name: 'parameter_group', type: 'STRING', description: "Group to which the GA4 parameter belongs."},
      {name: 'parameter_display_name', type: 'STRING', description: "Display name of the GA4 parameter."},
      {name: 'parameter_name', type: 'STRING', description: "Name of the GA4 parameter."},
      {name: 'parameter_scope', type: 'STRING', description: "Scope of the GA4 parameter."},
      {name: 'parameter_type', type: 'STRING', description: "Data type of the GA4 parameter."},
      {name: 'parameter_format', type: 'STRING', description: "Format of the GA4 parameter."},
      {name: 'parameter_disallow_ads_personalization', type: 'BOOL', description: "Indicates whether the GA4 parameter disallows ads personalization."},
      {name: 'parameter_example_value', type: 'STRING', description: "Example value for the GA4 parameter."},
      {name: 'parameter_description', type: 'STRING', description: "Description of the GA4 parameter."},
      {name: 'parameter_ga4_api_resource_name', type: 'STRING', description: "Name of the GA4 API resource associated with the parameter."},
      {name: 'parameter_gtm_comment', type: 'STRING', description: "Comments related to the GA4 parameter in Google Tag Manager."},
      {name: 'parameter_uploaded_to_bq_time', type: 'DATETIME', description: "Timestamp indicating when the GA4 parameter was uploaded to BigQuery."},
      {name: 'parameter_web', type: 'BOOL', description: "Indicates whether the GA4 parameter is used on the web."},
      {name: 'parameter_ios', type: 'BOOL', description: "Indicates whether the GA4 parameter is used on iOS."},
      {name: 'parameter_android', type: 'BOOL', description: "Indicates whether the GA4 parameter is used on Android."}
    ]
  };
  ...
  const annotationSchema = {
    fields: [
      ...
      {name: 'annotation_date', type: 'DATE', description: "Annotation Date."},
      {name: 'annotation_date_end', type: 'DATE', description: "Annotation End Date if GA4 Annotation date range."},
      {name: 'annotation_added_by_email', type: 'STRING', description: "Who added the annotation (if available)."},
      {name: 'annotation_category', type: 'STRING', description: "Annotation Category."},
      {name: 'annotation_color', type: 'STRING', description: "Annotation Color. Only for GA4 Annotations."},
      {name: 'annotation_description', type: 'STRING', description: "Annotation Description. Can be a combination of Title and Description depending on the API."},
      {name: 'annotation_ga4_gtm_info', type: 'STRING', description: "GA4 Property or GTM Container the annotation is relevant for."},
      {name: 'annotation_uploaded_to_bq_time', type: 'DATETIME', description: "When was the annotation uploaded to BigQuery."}
    ]
  };
...
if (sheetName === 'BigQuerySettings') {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const annotation_time = Utilities.formatDate(rows[i][annotationDateColumn-1], timezone, dateFormat);
      const annotation_date = Utilities.formatDate(rows[i][annotationDateColumn-1], timezone, dateFormat);
      const annotation_date_end = rows[i][annotationDateEndColumn-1] ? Utilities.formatDate(rows[i][annotationDateEndColumn-1], timezone, dateFormat) : '';
      ...
      const annotation_color = rows[i][annotationColorColumn-1].trim();
      ...
      report.push([
        annotation_time, annotation_date, annotation_date_end, annotation_added_by_email, annotation_category, annotation_color, annotation_description, annotation_ga4_gtm_info, annotation_uploaded_to_bq_time
      ]);
    }
  } else {
...
function uploadBigQuerySettingsToBigQuery() {
  ...
  try {
    ...
    if (!manualBQTrigger) {
      ss.toast(
        'BigQuery settings successfully uploaded to BigQuery.',
        'Upload complete',
        5
      );
    }
...
function uploadEventsToBigQuery() {
  ...
  // Settings upload (has its own toast only if not manual)
  uploadBigQuerySettingsToBigQuery();

  // Parameters upload (part of event upload flow)
  var parametersUploaded = uploadParametersToBigQuery();
  ...
  // Events upload
  var eventsUploaded = uploadToBigQuery(sheetName, tableId);

  // Return overall success
  return parametersUploaded && eventsUploaded;
}
...
function uploadParametersToBigQuery() {
  ...
  return uploadToBigQuery(sheetName, tableId);
}
