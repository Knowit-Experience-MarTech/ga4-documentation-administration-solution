...
* [**Make a copy of the Google Sheet**](https://docs.google.com/spreadsheets/d/1tnylUh55RJQf1UtCL1UaGiZV8ZrU1fgpaN_PbfP5KD0/copy)
...
## Annotations
Annotation in this documentation solution is more than just **GA4 Annotations**. In addition, you can download **GA4 Change History** and **GTM Container Versions**.

GA4 Annotations can also be added, edited or deleted using the Sheet.

### GA4 Annotations
**GA4 Account** and **Property** must have been selected in the **Settings** Sheet.

* You can create, edit or delete a GA4 Annotation from the Google Sheet.
* GA4 Annotations can be downloaded manually from the **GA4 Documentation** menu (📈 GA4 Documentation -> Annotations -> Get GA4 Annotations), or automatically using a [**Trigger**](#triggers).
...
* GA4 Change History can be downloaded manually from the **GA4 Documentation** menu (📈 GA4 Documentation -> Annotations -> Get GA4 Change History), or automatically using a [**Trigger**](#triggers).
...
The [**GTM API**](https://developers.google.com/tag-platform/tag-manager/api/v2) do not have any information about **who** made the change/published the container, so the text **Not Available from API** will be returned instead.

Suggested solution for downloading **GTM Container Versions** as Annotations:
* **Download Container Versions manually** the first time.
   *  Container Versions can be downloaded manually from the **GA4 Documentation** menu (📈 GA4 Documentation -> Annotations -> Get GTM Container Versions).
...
| **RegEx Validation** |
| Enforce snake_case | RegEx for enforcing **snake_case**. | ^[a-z_0-9]+(?:_?[a-z_0-9]+)*$ |
| Parameter Display Name | Parameter Display Name in "Parameters" Tab can only contain text, underscore and spaces. This RegEx enforces that for the English language, but you may be able to use characters from other alphabets as well. Change the RegEx to match your language if your documentation isn't in English. | ^[A-Za-z0-9_]+(?:[\s_]+[A-Za-z0-9_]+)*$ |
...
| Events Explained by Sessions Threshold | 0.1 | If an event anomaly is reported, and should have been explained by changes in sessions, increase the number. Decrease the number for the opposite scenario. Declared in query as **events_explained_by_sessions_threshold**. |
| Parameters Explained by Sessions Threshold | 0.1 | If an parameter anomaly is reported, and should have been explained by changes in sessions, increase the number. Decrease the number for the opposite scenario. Declared in query as **parameters_explained_by_sessions_threshold**. |
...
| ------------- | ------------- | ------------- | ------------- | ------------- |
| uploadAnnotationsToBigQuery | Time-drive | Hour timer | Every 2 hour | Uploads **Annotations** to BigQuery every 2 hour. |
| getGA4Annotations | Time-drive | Hour timer | Every 2 hour | Downloads **GA4 Annotations** to the Google Sheet every 2 hour. |
